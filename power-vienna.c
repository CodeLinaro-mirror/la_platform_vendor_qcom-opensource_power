	...
	switch(hint) {
		case POWER_HINT_INTERACTION:
			...
			if(enabled > 1) {
				...
				duration = enabled;
				interaction_with_handle(POWERHINT_INTERACTION_BOOST, duration, sizeof(resources)/sizeof(resources[0]), resources);
			}
			/* Interaction Hint for setMode*/
			else {
				if(enabled == 1) {
					int resources[] = {0x40800000, 0x1CC};
					perform_hint_action(POWERHINT_INTERACTION_MODE_ENABLED, resources, sizeof(resources)/sizeof(resources[0]));
				} else {
					undo_hint_action(POWERHINT_INTERACTION_MODE_ENABLED);
		...
		case POWER_HINT_LAUNCH:
			if(enabled) {
				int resources[] = {0x40804000, 0x7A6, 0x40804200, 0x840, 0x41000200, 0x0, 0x41004200, 0x1,
				                   0x40800000, 0xFFF, 0x40800200, 0xFFF, 0x41000000, 0x4, 0x40CA4000, 0x2,
